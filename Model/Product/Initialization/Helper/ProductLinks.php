...
namespace OuterEdge\AutoBuyRelated\Model\Product\Initialization\Helper;

use OuterEdge\AutoBuyRelated\Model\Catalog\Product\Link;
...
class ProductLinks
{
    ...
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getAutobuyReadonly()) {

            $links = (isset($links[self::TYPE_NAME])) ? $links[self::TYPE_NAME] : $product->getAutobuyLinkData();
            if (!is_array($links)) {
                $links = [];
            }

            if ($product->getAutobuyLinkData()) {
                $links = array_merge($links, $product->getAutobuyLinkData());
            }
            $newLinks = [];
